...
<?php if($Auth->user('admin')) { ?>            
                ...
                <li>
                    <?= $this->Html->link(__('New {0}', ['Categories Product']), ['action' => 'add']) ?>
                </li>
                <li>
            ...
            <div class="box-body">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <?= $this->Number->format($categoriesProduct->id) ?>
                            </td>
                            <td>
                                <?= $categoriesProduct->has('user') ? $this->Html->link($categoriesProduct->user->name, ['controller' => 'Users', 'action' => 'view', $categoriesProduct->user->id]) : '' ?>
                            </td>
                            <td>
                                <?= $categoriesProduct->has('category') ? $this->Html->link($categoriesProduct->category->title, ['controller' => 'Categories', 'action' => 'view', $categoriesProduct->category->id]) : '' ?>
                            </td>
                            <td>
                                <?= $categoriesProduct->has('product') ? $this->Html->link($categoriesProduct->product->title, ['controller' => 'Products', 'action' => 'view', $categoriesProduct->product->id]) : '' ?>
                            </td>
